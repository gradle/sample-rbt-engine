...
This project is similar to the main demo, and uses the same engine, but does not propagate the TestEngines's output and instead uses a custom TestListener to demonstrate how tests can be listened for in Gradle builds.
...
(The "UnknownClass" in the output is a known issue, that will be corrected in the next Gradle milestone.)

### No Tests Discovered Demo `demo-m4-no-tests`

This project is similar to the main demo, and uses the same engine, but does not supply any valid test definition files for that engine.

Run the demo project using `./gradlew :demo-m4-no-tests:test --rerun` and you'll see the task fail with a message indicating no tests were found:

```text
> Task :demo-m4-no-tests:test FAILED

FAILURE: Build failed with an exception.

* What went wrong:
Execution failed for task ':demo-m4-no-tests:test'.
